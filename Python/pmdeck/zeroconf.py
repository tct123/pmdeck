...
def service_type_name(type_, *, allow_underscores: bool = False):
    ...
    """
    if not (type_.endswith('._tcp.') or type_.endswith('._udp.')):
        raise BadTypeInNameException(
            "Type '%s' must end with '._tcp.' or '._udp.'" %
            type_)

    remaining = type_[:-len('._tcp.')].split('.')
    name = remaining.pop()
    if not name:
...
class ZeroconfServiceTypes:
    ...
    def find(cls, zc=None, timeout=5, interfaces=InterfaceChoice.All):
        ...
        browser = ServiceBrowser(
            local_zc, '_services._dns-sd._udp.', listener=listener)
    ...
    def handle_query(self, msg: DNSIncoming, addr: str, port: int) -> None:
        ...
        for question in msg.questions:
            if question.type == _TYPE_PTR:
                if question.name == "_services._dns-sd._udp.":
                    for stype in self.servicetypes.keys():
                        if out is None:
                            out = DNSOutgoing(_FLAGS_QR_RESPONSE | _FLAGS_AA)
                        out.add_answer(msg, DNSPointer(
                            "_services._dns-sd._udp.", _TYPE_PTR,
